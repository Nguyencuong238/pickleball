                ...
                <form action="{{ route('admin.videos.update', $video->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="instructor_id" id="instructor_id" class="form-control @error('instructor_id') is-invalid @enderror">
                            ...
                            @foreach($instructors as $instructor)
                                <option value="{{ $instructor->id }}" {{ old('instructor_id', $video->instructor_id) == $instructor->id ? 'selected' : '' }}>
                                    {{ $instructor->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="video_link" id="video_link" class="form-control @error('video_link') is-invalid @enderror"
                            value="{{ old('video_link', $video->video_link) }}" placeholder="https://www.youtube.com/watch?v=... hoặc https://youtube.com/shorts/...">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="duration" id="duration" class="form-control @error('duration') is-invalid @enderror"
                            value="{{ old('duration', $video->duration) }}" placeholder="25:30">
                        @error('duration')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="level" id="level" class="form-control @error('level') is-invalid @enderror">
                            ...
                            <option value="Người mới" {{ old('level', $video->level) == 'Người mới' ? 'selected' : '' }}>Người mới</option>
                            <option value="Trung bình" {{ old('level', $video->level) == 'Trung bình' ? 'selected' : '' }}>Trung bình</option>
                            <option value="Nâng cao" {{ old('level', $video->level) == 'Nâng cao' ? 'selected' : '' }}>Nâng cao</option>
                            <option value="Chuyên gia" {{ old('level', $video->level) == 'Chuyên gia' ? 'selected' : '' }}>Chuyên gia</option>
                        </select>
                        @error('level')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="views_count" id="views_count" class="form-control @error('views_count') is-invalid @enderror"
                            value="{{ old('views_count', $video->views_count) }}" min="0" placeholder="0">
                        @error('views_count')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="rating" id="rating" class="form-control @error('rating') is-invalid @enderror"
                            value="{{ old('rating', $video->rating) }}" min="0" max="5" step="0.1" placeholder="4.9">
                        @error('rating')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="rating_count" id="rating_count" class="form-control @error('rating_count') is-invalid @enderror"
                            value="{{ old('rating_count', $video->rating_count) }}" min="0" placeholder="234">
                        @error('rating_count')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                            ...
                            @if ($video->chapters && is_array($video->chapters) && count($video->chapters) > 0)
                                @foreach ($video->chapters as $chapter)
                                    <div class="chapter-input-group mb-2">
                                        <div class="row">
                                            <div class="col-md-3">
                                                <input type="text" class="form-control chapter-time" placeholder="00:00" value="{{ $chapter['start_time'] ?? '' }}">
                                            </div>
                                            <div class="col-md-6">
                                                <input type="text" class="form-control chapter-title" placeholder="Tiêu đề chương" value="{{ $chapter['title'] ?? '' }}">
                                            </div>
                                            <div class="col-md-2">
                                                <input type="text" class="form-control chapter-duration" placeholder="3:00" value="{{ $chapter['duration'] ?? '' }}">
                                            </div>
                                            <div class="col-md-1">
