    ...
    <div style="margin-bottom: 15px;">
        <div class="gallery-media">
            @forelse ($existingMedia as $media)
                <div class="gallery-media-item" id="existing-media-{{ $media->id }}" data-media-id="{{ $media->id }}">
                    <img src="{{ $media->getUrl() }}" alt="media-{{ $media->id }}">
                    <button type="button" class="btn-delete-media btn-delete-existing" data-id="{{ $media->id }}">
                        ✕ Xóa
                    </button>
                </div>
            @empty
