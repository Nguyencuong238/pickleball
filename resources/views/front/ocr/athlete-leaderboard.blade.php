...
<style>
    ...
    .pagination-nav .pagination li:not(.disabled):not(.active) .page-link:hover {
        border-color: var(--primary-color);
    ...
    .pagination-nav .pagination li.disabled .page-link {
        opacity: 0.5;
        cursor: not-allowed;
        color: #cbd5e1;
    }
        ...
        <div class="page-header-content">
            <div>
                <p class="page-breadcrumb">
                 ...
                 <h1 class="page-title">🏆 Bảng Xếp Hạng VĐV</h1>
            </div>
        </div>
    </div>
</section>
...
<section class="leaderboard-section">
    <div class="container">
        ...
        <div class="type-filter-section">
            @foreach($athleteTypes as $typeKey => $typeLabel)
                <a href="{{ route('athlete-leaderboard', ['type' => $typeKey]) }}" 
                   class="type-filter-tab {{ $type === $typeKey ? 'active' : '' }}">
                    {{ $typeLabel }}
                </a>
            @endforeach
        </div>
            ...
            @if($userRank)
                <div class="user-rank-banner">
                    <div class="user-rank-info">
                         <div>
                            <div class="user-stat-value">Thông Số Của Bạn</div>
                         </div>
                     </div>
        ...
        <div class="filter-section">
            <a href="{{ route('athlete-leaderboard', ['type' => $type]) }}" 
               class="filter-tab {{ !$rank ? 'active' : '' }}">
                Tất Cả Hạng
            </a>
            @foreach($ranks as $r)
                <a href="{{ route('athlete-leaderboard', ['type' => $type, 'rank' => strtolower($r)]) }}"
        ...
        <div class="leaderboard-card">
                    ...
                    @empty
                        <tr>
                             <td colspan="5" style="text-align: center; padding: 3rem; color: #94a3b8;">
                                 @if($rank)
                                     Chưa có {{ strtolower($athleteTypes[$type]) }} nào ở hạng {{ ucfirst($rank) }}
                                 @else
                                     Chưa có dữ liệu xếp hạng cho {{ strtolower($athleteTypes[$type]) }}
                                 @endif
                             </td>
                         </tr>
                    @endforelse
            ...
            @if($players->hasPages())
                <div class="pagination-wrapper">
                    {{ $players->appends(['type' => $type, 'rank' => $rank])->links('vendor.pagination.athlete-leaderboard') }}
                </div>
            @endif
        </div>
    </div>
